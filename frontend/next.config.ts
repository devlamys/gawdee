...
// Quick Tunnel URLs are subdomains; the bare trycloudflare.com host does not match them.
const devOrigins = (process.env.ALLOWED_DEV_ORIGINS || "localhost,127.0.0.1,192.168.29.61,*.trycloudflare.com")
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);
